(function () {
				...
				for (var i = 0; i < 10; i++)
					for (var j = 0; j < 10; j++)
						if(battle.defender.set.map[i][j] == 2)
							$(opponent_map[i][j].context.firstChild).addClass("glyphicon-remove");
						else if(battle.defender.set.map[i][j] == 3)
							$(opponent_map[i][j].context.firstChild).addClass("glyphicon-certificate");
	...
	$("#opponent button").on("click", function () {
		...
		var didCorrectShot = false, canHitAgain = false;

		while(!didCorrectShot){
